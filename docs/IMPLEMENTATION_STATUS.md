...
### 0-3. 未pushのコミットが6件あります

```
f71d76d refactor(security): パスワード設計を見直し、認証経路をTOTP一本に統一
54b0162 docs: 未追跡の一時ファイル2件を削除し、チェックリストの5-3を対応済みに更新
e90fe31 docs: SETUP_NEW_PC.md・backup-guide.htmlを対策内容に合わせて更新し、命綱チェックリストを追加
22bd4e8 feat(security): 対策4 ログインをGoogle Authenticator(TOTP)方式に変更
12e7330 feat(security): 対策2 監査ログ(誰が・いつ・何を見たか)を実装
a2daf78 feat(security): 対策3(SQLi監査)・対策5(ネットワーク制限)・対策6(インシデント対応)を整備
```

**pushは現状失敗します。** remote URLにGitHubのアクセストークンが埋め込まれており、
gitがそれをユーザー名として解釈してパスワードを要求するためです。

Windows資格情報マネージャーには `github.com` / `nagasekioto` の有効な認証情報が
既に保存されているため、**remote URLからトークンを外せばpushは通ります**（検証済み）。

```powershell
git remote set-url origin https://github.com/nagasekioto/workOfficeTani.git
```

> この対応は利用者の指示により保留中です。実行前に確認してください。
...
| 4 | **GitHubトークンの失効** | `.git\config` に平文で残り、触れる人が誰でもリポジトリを書き換えられる |
...
2. **6件のコミットをpushする**（remote URLからトークンを外す必要がある）
