...
4. **認証チェックの集約**（2-2）— 52箇所をInterceptor1箇所へ。**CSRF対策より先に行うこと**
   （CSRFトークンの検証も同じInterceptorに載せるため。順序を逆にすると
   チェック機構が2つ並存して書き漏れの温床が増える）
5. **CSRF対策**（2-2）— 4の基盤に載せる。`SameSite=Strict`（1-5-1）は緩和策であって代替ではない
6. **対策1 DBロール分離**（2-1）— 起動不能リスクが高いので段階を分けて
