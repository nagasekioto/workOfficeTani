...
アプリ自体も既定でlocalhost(127.0.0.1)にしかバインドされないため、これと合わせて二重に防御されます（→ 第5章）。
...
## 5. アプリ側のバインドアドレス制限（既定で有効・4-2-Aの場合）

`application.yml` で、アプリが待ち受けるアドレスを環境変数で制御しています。
...
**現在の既定値は `127.0.0.1`（このPC専用）です。** 4-2-A（このPCだけで使う）運用であれば、
環境変数を何も設定しなくてもそのまま安全な状態で動作します。

4-2-B（社内の他PCからも使う）運用に変える場合のみ、環境変数を設定してください。
...
（`SERVER_ADDRESS` を設定していない、または未設定の既定状態の場合）。

```powershell
netstat -ano | Select-String ":8080"
```

> ⚠️ `SERVER_ADDRESS=0.0.0.0` に設定して他PCからも使う運用にする場合は、
> 必ず4-2-Bのファイアウォール設定（社内サブネットのみ許可）を行ってください。
> 設定しないと、社内ネットワーク上の誰からでもアプリ画面へアクセスできてしまいます。
